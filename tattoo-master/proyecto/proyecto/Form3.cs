...
namespace proyecto
{
    public partial class Form3 : Form
    {
        ...
        public Form3()
        {
            ...
            MostrarDatoscita();
            MostrarDatostattoo();
            MostrarDatosreportecliente();
            MostrarDatosreportetatuador();
        ...
        private void CaptureScreen()
        {
            ...
            memoryGraphics.CopyFromScreen(panel16.Location.X,panel16.Location.Y,-130,130,s);
        }
...
this.tatuajeTableAdapter.Fill(this.bd.tatuaje);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (f == 1)
            {
                ...
                {
                    conexion_bd.consulta("SELECT * FROM cita WHERE id_cita='" + i + "'", "cita");
                    //conexion_bd.consulta("SELECT cliente.nombre, cita.id_cita FROM cita,cliente WHERE cliente.id_cliente = cita.id_cliente AND id_cita='" + i + "'", "cita,cliente");
                    datos.DataSource = conexion_bd.ds.Tables["cita"];
                    //datoscliente.DataSource = conexion_bd.ds.Tables["cita,cliente"];


                    string tatuadordelacita;
                    int elcarnetdeidentidad;
                    String elnombredelcliente="";
                    int elano;
                    int elmes;
                    int eldia;
                    int horaini1;
                    ...
                    tatuadordelacita = Convert.ToString (datos.CurrentRow.Cells[1].Value.ToString());//seleccionar el usuario de la base de datos from citas where id_citas=i
                    elcarnetdeidentidad = Convert.ToInt32 (datos.CurrentRow.Cells[2].Value.ToString()); //seleccionar el carnet de identidad del cliente de la cita de la base de datos from citas where id_citas=i
                    //elnombredelcliente = Convert.ToString(datoscliente.CurrentRow.Cells[0].Value.ToString()); //seleccionar el nombre del cliente de la cita de la base de datos from CLIENTE where id_cliente = i
                    elano = Convert.ToInt32 (datos.CurrentRow.Cells[3].Value.ToString()); //seleccionar el año de la base de datos from citas where id_citas=i
                    elmes = Convert.ToInt32 (datos.CurrentRow.Cells[4].Value.ToString()); //seleccionar el mes de la base de datos from citas where id_citas=i
                    eldia = Convert.ToInt32 (datos.CurrentRow.Cells[5].Value.ToString()); //seleccionar el dia de la base de datos from citas where id_citas=i
                    horaini1 = Convert.ToInt32 (datos.CurrentRow.Cells[6].Value.ToString()); //seleccionar la hora de inicio de la base de datos from citas where id_citas=i
                    horafin1 = Convert.ToInt32 (datos.CurrentRow.Cells[8].Value.ToString()); //seleccionar la hora de inicio de la base de datos from citas where id_citas=i
                    minini1 = Convert.ToInt32 (datos.CurrentRow.Cells[7].Value.ToString()); //seleccionar el minuto de inicio de la base de datos from citas where id_citas=i
                    minfin1 = Convert.ToInt32 (datos.CurrentRow.Cells[9].Value.ToString()); //seleccionar el minuto de inicio de la base de datos from citas where id_citas=i
                    //MessageBox.Show(Convert.ToString( tatuadordelacita));
                    //MessageBox.Show(Convert.ToString( elcarnetdeidentidad));
                    ////MessageBox.Show(elnombredelcliente);
                    //MessageBox.Show(Convert.ToString( elano));
                    //MessageBox.Show(Convert.ToString( elmes));
                    //MessageBox.Show(Convert.ToString( eldia));
                    //MessageBox.Show(Convert.ToString( horaini1));
                    //MessageBox.Show(Convert.ToString( horafin1));
                    //MessageBox.Show(Convert.ToString( minini1));
                    //MessageBox.Show(Convert.ToString( minfin1));
                    //convertir todos a enteros
                    if ((Convert.ToInt32(anocita.Text) == Convert.ToInt32 (elano)) && (Convert.ToInt32(mescita.Text) == Convert.ToInt32 (elmes)) && (Convert.ToInt32(diacita.Text) == Convert.ToInt32(eldia)))
                    {
                        ...
                        if ((HI1 < HI2 && HI2 < HF1) || (HI1 < HF2 && HF2 < HF1) || (HI2 <= HI1) && (HF1 <= HF2) && (tatuadordelacita == tatuadorcita.SelectedValue))
                        { //NO SE SI ESE ES CODIGO PARA OBTENER EL TATUADOR DEL COMBOBOX
                    ...
                    {
                        if (Convert.ToString(elcarnetdeidentidad) == cicita.Text)
                        {
                            if (elnombredelcliente == nombrecita.Text && Convert.ToString(elcarnetdeidentidad) == cicita.Text)
                            {
       ...
       (MessageBox.Show("Seguro que desea eliminar?", "ELIMINAR", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
        ...
        {
            reportecliente.Visible = true;
            reporte_tatuadores.Visible = false;
            msgcliente.Visible = true;
            msgtattoo.Visible = false;
            msgcliente.Text="EL SIGUIENTE REPORTE ES DE LOS CLIENTES CON MAS ASISTENCIA AL 'ESTUDIO' DE TATUAJES ORDENADOS DE FORMA DESCENDENTE";
        }
        ...
        private void reportecliente_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }
        public void MostrarDatosreportecliente()
        {
            //metodo para mostrar datos(USUARIO)
            conexion_bd.consulta("SELECT COUNT(cita.id_cliente) AS N_citas, cliente.nombre, cliente.id_cliente AS CI , SUM(tatuaje.precio) AS precio_tatuaje_total FROM cita, cliente, tatuaje WHERE cliente.id_cliente = cita.id_cliente AND cita.id_tatuaje = tatuaje.id_tatuaje GROUP BY (cliente.id_cliente)", "cita");
            reportecliente.DataSource = conexion_bd.ds.Tables["cita"];
            

        }
        public void MostrarDatosreportetatuador()
        {
            //metodo para mostrar datos(USUARIO)
            conexion_bd.consulta("SELECT COUNT(cita.id_usuario) AS n_citas, usuario.nombreusu AS Nombre ,usuario.apellidousu AS Apellido, usuario.id_usuario AS CI_Tatuador FROM cita, usuario WHERE usuario.id_usuario= cita.id_usuario GROUP BY (cita.id_usuario)", "cita");
            reporte_tatuadores.DataSource = conexion_bd.ds.Tables["cita"];
        ...
        private void button4_Click_2(object sender, EventArgs e)
        {
            ...
            reporte_tatuadores.Visible = true;
            reportecliente.Visible = false;
            msgtattoo.Text = "EL SIGUIENTE REPORTE ES DE LOS TATUADORES Y LA CANTIDAD DE CITAS QUE TIENE CADA UNO EN EL MOMENTO DE EJECUTAR EL REPORTE";
            msgtattoo.Visible = true;
            msgcliente.Visible = false;
        }
